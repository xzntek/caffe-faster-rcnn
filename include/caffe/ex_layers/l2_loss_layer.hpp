#ifndef CAFFE_L2_LOSS_LAYER_HPP_
#define CAFFE_L2_LOSS_LAYER_HPP_
...
/**
 ** For Zero Input
 * @brief Computes the L2 (L2) loss @f$
 ...
 *      the computed L2 loss: @f$ E =
 *          \frac{1}{2n} \sum\limits_{n=1}^N \left| \left| \hat{y}_n - y_n
 *        \right| \right|_2^2 @f$
 *
 * This can be used for least-squares regression tasks.  An InnerProductLayer
 * input to a L2LossLayer exactly formulates a linear least squares
...
class L2LossLayer : public LossLayer<Dtype> {
 public:
  explicit L2LossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), dot(0) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "L2Loss"; }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  /**
   * Unlike most loss layers, in the L2LossLayer we can backpropagate
 ...
 protected:
  ...
  /**
   * @brief Computes the L2 error gradient w.r.t. the inputs.
   *
   * Unlike other children of LossLayer, L2LossLayer \b can compute
